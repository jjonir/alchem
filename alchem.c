...
int main() {
	...
	m = new_manipulator(PX, 2, pos(1, 0, 0));
	...
	m = new_manipulator(PX, 2, pos(0, 2, 0));
	inst_add(m, OPEN_HEAD);
	inst_add(m, RETRACT);
	inst_add(m, CLOSE_HEAD);
	inst_add(m, EXTEND);
...
*/
	g = new_glyph(SOURCE, (uint8_t)ATOM_AL, pos(3, 0, 0));
	add_glyph(w, g, pos(3, 0, 0));
	g = new_glyph(BOND, (uint8_t)PX, pos(1, 2, 0));
	add_glyph(w, g, pos(1, 2, 0));
